...
def big_text(text, width = 25, key=None):
    words = text.split(' ')
    i = 1
    next_len = 0
    a_text = str.capitalize(words[0])
...
def make_actionbox(is_train):
    box_layout = [
        [sg.Button('Select Image', key='select_img', p=5), sg.Button('Continue', p=5)],
        [sg.Checkbox('For Training', key='training', default=is_train), sg.Button('Save + Exit', p=5),sg.Button('Remove', p=5)]
    ]
...
def goto_overview(window, project):
    layout= [
        ...
        [sg.Table(project.get_df_as_list(),
                            ...
                            justification='left',
                            k='table' )], 
        [sg.Button('Continue', k='o_cont'), sg.Button('Edit'), sg.Button('Create tasklist', k='create_exp'), sg.Button('Generate Experiment', k='gen'), sg.Button('Generate tasklist v2', k='gen2'), sg.Button('Generate Code v2', k="create_exp_code")]
        ]
    window.close()
...
def goto_annotate(window, project):
    ...
    hyper,bl_syn,bl_def,syns,bl_name, hypo, hypo_img, hypo_syns, hypo_def, certainty, is_train = str(info[0]),str(info[1]),str(info[2]),str(info[3]), str(info[4]), str(info[5]), str(info[6]), str(info[7]), str(info[8]), str(info[9]), str(info[10])
    ...
    actbox = make_actionbox(is_train)
    layout = [
        [sg.Menu(menu_def, key='menu')],
...
def main():
    ...
    while True:      
        ...
        if event == 'About':      
            ...
            window['hypo_syns'].Update(paragrapher(syns, 25))
            window['hypo_def'].Update(paragrapher(hypodef, 25))
            ...
            cwd = window.find_element('dir').Get()
            identifier = os.path.basename(cwd)
            ...
            success = experiment_code.generate()
            if success:
                experiment_code.store_tt()
            else:
                sg.popup_annoying("Generate again!")

        elif event == "gen2":
            success = experiment_code.gen_v2()
            if success:
                experiment_code.store_tt(option="multi")
            else:
                sg.popup_annoying("Generate again!")
            
        elif event == "create_exp_code":
            experiment_code.gen_experiment_v2()

        elif event == "create_exp":
            experiment_code = tc.experiment_creator(project.id, project.cwd, project.df)

        elif event == "training":
            project.set_training_flag()
