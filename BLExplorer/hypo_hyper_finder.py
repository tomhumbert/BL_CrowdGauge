...
class BL_EXP:
    ...
    def add_img(self, image_name):
        ind = self.get_bl_index(self.last)
        self.df.at[ind,'hyponym_img'] = image_name
        return True
    ...
    def create_new_df(self, bl_list):
        ...
        train_col = [False for i in range(l)]
        cont_col[0] = True
    ...
    def next(self):
        df_max_i = len(self.df.index)-1
        last_index = self.get_bl_index(self.last)
        index = 0
        if last_index+1 <= df_max_i:
            index = last_index+1
    ...
    def get_def_info(self):
        ...
        is_train = list(row.get('for_training').values())[0]
        if hypo != 'missing':
            ...
            hypo_syns = ['missing']
            hypo_def = "missing"
        return [hyper,bl_syn,bl_def,syns,bl_name, hypo, hypo_img, hypo_syns, hypo_def, certainty, is_train]
    ...
    def set_training_flag(self):
        last, index = self.set_last_stored_pos
        self.df["for_training"][index] = self.df["for_training"][index] == False
