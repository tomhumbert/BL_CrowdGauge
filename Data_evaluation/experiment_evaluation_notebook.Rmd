...
acc_by_participant <- ggplot(participant_means, 
                             aes(x=reorder(p_id, desc(-mean_acc)), y=mean_acc))+
  geom_point() +
...
rt_by_participant <- ggplot(participant_means, 
                             aes(x=reorder(p_id, desc(-mean_rt)), y=mean_rt))+
  geom_point() +
  xlab("Participant number") +
...
stimOV <- exp_results %>%
  group_by(stimulus,accurate, index) %>%
  summarise(accuracy = mean(accurate),n=n())

plot_mistake_overview <- ggplot(data=stimOV, aes(x=index,y=n, fill=as.factor(accurate))) +
  geom_col(width = 1) +
  theme(axis.text.x = element_blank(), 
        axis.ticks.x = element_blank(),
        legend.position = 0)+
  xlab("Stimuli ordered as they appear in experiment") +
  ylab("Counts")
...
grid.arrange(plot_mistake_overview,plot_mistake_overview_dist, widths=c(5,2))
...
m_by_branch_plot <- ggplot(mistake_by_branch, aes(x=error, y=reorder(branch, desc(-error))), group=lvl) +
  ...
  theme(axis.text.y = element_text(angle=0),
        ...
        panel.grid.minor = element_line(size=0.02, colour = "#c0c0c0")) +
  scale_x_continuous(labels=scales::percent) +
  labs(title="Accuracy by branches.", fill="Level") +
  xlab("Accuracy") +
